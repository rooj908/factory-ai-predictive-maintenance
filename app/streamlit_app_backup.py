...
st.markdown(
    '<div class="main-title">🏭 Factory AI Predictive Maintenance</div>',
    unsafe_allow_html=True
)

st.markdown(
    '<div class="subtitle">'
    'Multi-Agent AI System for Predictive Maintenance, Computer Vision, '
    'RAG and Digital Twin Simulation'
    '</div>',
    unsafe_allow_html=True
)
...
st.sidebar.markdown("### Machine Sensors")
...
def sensor_input(column, default):
...
st.sidebar.markdown("---")
st.sidebar.markdown("### 📷 Machine Image")
...
query = st.sidebar.text_area(
    ...
    value="Machine has high vibration and needs maintenance."
)
...
if run_analysis:
    ...
    failure_probability = make_prediction()
    ...
    planning_result = {
        ...
        "priority": risk_level,
        "recommendation": (
            "Inspect machine before returning it "
            "to full production."
            if risk_level == "HIGH"
            else
...
with tab2:
    ...
    if uploaded_image:
        ...
        st.info(
            "📷 Upload a machine image from the sidebar "
            "to activate the Computer Vision Agent."
        )
...
with tab3:
    ...
    st.header(
        "3. RAG / Maintenance Knowledge"
    )

    st.write(
        "Maintenance evidence retrieved from the "
        "knowledge base."
    )
    ...
    if evidence:
        ...
        st.info(
            "Run AI Analysis to retrieve maintenance evidence."
        )
...
with tab5:
    ...
    st.header(
        "5. Factory Digital Twin"
    )

    st.write(
        "Simulate different maintenance strategies "
        "before taking action on the real machine."
    )
    ...
    digital_twin_df = pd.DataFrame(
        [
            {
                "Scenario": "Continue Operation",
                "Downtime Hours": continue_downtime,
                "Failure Risk": round(
                    continue_risk,
                    2
                ),
                "Production Loss": continue_loss,
            ...
            {
                "Scenario": "Stop for Maintenance",
                "Downtime Hours": maintenance_downtime,
                "Failure Risk": maintenance_risk,
                "Production Loss": maintenance_loss,
                "Estimated Cost": round(
                    maintenance_cost,
                    2
                )
            ...
            {
                "Scenario": "Reduce Machine Load",
                "Downtime Hours": reduced_downtime,
                "Failure Risk": round(
                    reduced_risk,
                    2
                ),
                "Production Loss": reduced_loss,
                "Estimated Cost": round(
                    reduced_cost,
                    2
                )
    ...
    best_scenario = digital_twin_df.loc[
        digital_twin_df["Estimated Cost"].idxmin()
    ]
...
st.caption(
    ...
    "Predictive AI + Computer Vision + RAG + "
